...
void GetJPEGDimensions(const char *filename, int &w, int &h)
{
    ...
    if ((f = fopen(filename, "rb")) == NULL) {
        LOGI("[GetJPEGDimensions] Error: can't open file %s for reading\n", 
               filename);
        return;
    }
    ...
    LOGI("[GetJPEGDimensions] File %s: ( %d , %d )\n", filename, w, h);
...
img_t *LoadJPEG(const char *filename)
{
    ...
    if ((f = fopen(filename, "rb")) == NULL) {
        LOGI("[LoadJPEG] Error: can't open file %s for reading\n", filename);
        return NULL;        
    }
...
void WriteJPEG(const img_t *img, const char *filename)
{
    ...
    if ((outfile = fopen(filename, "wb")) == NULL) {
        LOGI("[WriteJPEG] can't open file %s for writing\n", filename);
        return;
    }
