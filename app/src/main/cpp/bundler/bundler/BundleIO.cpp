...
{
    LOGI("[SifterApp::LoadKeys] Loading keys...\n");
    ...
    for (int i = 0; i < num_images; i++) {
        LOGI("[SifterApp::LoadKeys] Loading keys from image %d...\n", i);
        fflush(stdout);
    ...
    LOGI("[SifterApp::LoadKeys] Loaded keys in %0.3fs\n", 
...
void BaseApp::LoadMatchTable(const char *filename) {
    ...
    LOGI("[LoadMatchTable] Loading matches\n");
    ...
    if (f == NULL) {
        LOGI("[LoadMatchTable] Error opening file %s for reading\n",
               filename);
        exit(1);
    }
...
void BaseApp::LoadMatchIndexes(const char *index_dir)
{
    ...
    for (int i = 0; i < num_images; i++) {
        ...
        LOGI("[LoadMatchIndexes] Loading matches for image %d... ", i);
        ...
        while(fgets(buf, 256, f) != NULL) {
            ...
            if (num_matches < MIN_MATCHES || index >= num_images) {
                if (index >= num_images) 
                    LOGI("[LoadMatchIndexes] image index %d > num_images\n",
                           index);
...
void BaseApp::LoadMatches() {
    ...
    if (m_match_table != NULL) {
        ...
        LOGI("[LoadMatches] Loading matches\n");

        int num_images = GetNumImages();

        FILE *f = fopen("/storage/sdcard/matcher/match-index.txt", "r");
        if (f == NULL) {
            /* Try all pairs */
            for (int i = 0; i < num_images; i++) {
            ...
            LOGI("[LoadMatches] Reading matches from 'match-index.txt'\n");
            fflush(stdout);
            ...
            LOGI("[LoadMatches] Read %d match files\n", count);
            fflush(stdout);
...
{
    LOGI("[SifterApp::ReadBundleFile] Reading file...\n");
    ...
    if (first_line[0] == '#') {
        ...
        LOGI("[ReadBundleFile] Bundle version: %0.3f\n", version);
    ...
    LOGI("[SifterApp::ReadBundleFile] Reading %d images and %d points...\n",
        num_images, num_points);

    if (num_images != GetNumImages()) {
        LOGI("Error: number of images doesn't match file!\n");
        return;
    }
                             ...
                             /*bool output_radial_distortion*/)
{
    ...
    char buf[256];
	//TODO
	//output_dir = "/storage/sdcard/matcher/bundle";
    sprintf(buf, "%s/%s", output_dir, filename);

    FILE *f = fopen(buf, "w");
    if (f == NULL) {
        LOGI("Error opening file %s for writing\n", buf);
        return;
    }
    ...
    LOGI("[SifterApp::DumpOutputFile] Wrote file in %0.3fs\n",
...
void BaseApp::WriteCamerasXML(const char *filename)
{
    ...
    if (f == NULL) {
	LOGI("[SifterApp::WriteCamerasXML] "
...
void BaseApp::WritePointsXML(const char *filename) 
{
    ...
    LOGI("[SifterApp::WritePointsXML] %d / %d points seen by >= %d views\n",
...
void BaseApp::ReadMatchTableDrew(const char *append) 
{
    ...
    char buf[256];
    sprintf(buf, "/storage/sdcard/matcher/nmatches%s.txt", append);
    FILE *f0 = fopen(buf, "r");

    sprintf(buf, "/storage/sdcard/matcher/matches%s.txt", append);
    FILE *f1 = fopen(buf, "r");
    ...
    LOGI("[ReadMatchTableDrew] Read %lu matches in total\n",
           num_matches_total);
                              ...
                              /*bool reflect*/) 
{
	...
	//TODO
	//output_directory = "/storage/sdcard/matcher/bundle";
...
{
    if (FileExists("/storage/sdcard/matcher/camera-constraints.txt")) {
	LOGI("[SifterApp::ReadCameraConstraints] Reading constraints\n");

	FILE *f = fopen("/storage/sdcard/matcher/camera-constraints.txt", "r");
	char buf[256];
...
void BaseApp::ReadPointConstraints()
{
    ...
    if (f == NULL) {
		LOGI("[SifterApp::ReadPointConstraints] Error opening file %s "
    ...
    while (fgets(buf, 256, f) != NULL) {
		...
		LOGI("[SifterApp::ReadPointConstraints] Constraining %d: "
...
{
    LOGI("[ReadIntrinsicsFile] Reading intrinsics...\n");
...
void BaseApp::ReadIgnoreFile()
{
    ...
    if (f == NULL) {
		LOGI("[SifterApp::ReadIgnoreFile] Error opening file %s "
			   "for reading\n", m_ignore_file);
		return;
    }
    ...
    while (fgets(buf, 255, f)) {
		...
		if (img < 0 || img >= num_images) {
			LOGI("[SifterApp::ReadIgnoreFile] "
			   "Error: image %d out of range\n", img);
			continue;
		}
		
		LOGI("[SifterApp::ReadIgnoreFile] Ignoring image %d\n", img);
...
void BaseApp::InitializeImagesFromFile(FILE *f) 
{
	...
	LOGI("[SifterApp::InitializeImagesFromFile] Initializing image %s\n",
	       data.m_name);
