...
static void RemoveImageKey(ImageKeyVector &vec, int view) 
{
    ...
    if (found == 0)
        LOGI("[RemoveImageKey] Error! Couldn't find view %d\n", view);
    else if (found > 1) 
        LOGI("[RemoveImageKey] Error! Found too many views %d\n", view);
}
                         ...
                         int *added_order)
{
    ...
    for (int i = 0; i < num_points; i++) {
        ...
        for (int j = 0; j < num_views; j++) {
            ...
            if (m_image_data[img].m_keys[key].m_extra != i) {
                LOGI("[CheckPointKeyConsistency] Error: (%d,%d).m_extra "
    ...
    LOGI("[CheckPointKeyConsistency] There were %d errors\n", errors);
}
...
void BundlerApp::ReRunSFM(double *S, double *U, double *V, double *W) 
{
    ...
    LOGI("[SifterApp::ReRunSFM] Initializing keypoints...\n");
    ...
    LOGI("[SifterApp::ReRunSFM] Setting up cameras\n");
    for (int i = 0; i < num_images; i++) {
        LOGI(".");
        fflush(stdout);
            ...
            if (m_constrain_focal) {
                ...
                if (m_image_data[i].m_has_init_focal) {
                    ...
                    if (fabs(diff) / m_image_data[i].m_init_focal < 0.4) {
                        LOGI("[ReRunSFM] Constraining focal "
                            "length for camera %d\n", i);
    ...
    LOGI("[ReRunSFM] Setting up views...\n");
    ...
    for (int i = 0; i < num_pts; i++) {
        ...
        for (int j = 0; j < num_views; j++) {
            ...
            if (m_image_data[v].m_keys[k].m_extra != -1) {
                LOGI("Error! Already assigned this key "
                    "[%d,%d] <- %d != %d!\n", v, k, 
...
#define MIN_POINTS 20
    ...
    do {
        if (num_pts - total_outliers < MIN_POINTS) {
            LOGI("[RunSFM] Too few points remaining, exiting!\n");
            fflush(stdout);
        ...
        LOGI("[RunSFM] run_sfm took %0.3fs\n",
...
#define NUM_STDDEV 2.0 // 3.0 // 6.0
            ...
            * camera */
            ...
            LOGI("[RunSFM] Mean error cam %d[%d] [%d pts]: %0.3e "
                "[med: %0.3e, %0.3e]\n",
            ...
            // LOGI("Outlier threshold is %0.3f\n", thresh);
...
#define NUM_ERROR_BINS 10
        ...
        if (remove_outliers) {
            for (int i = 0; i < (int) outliers.size(); i++) {
                ...
                LOGI("[RunSFM] Removing outlier %d "
                    "(reproj error: %0.3f)\n", idx, reproj_errors[i]);
            ...
            LOGI("[RunSFM] outlier removal took %0.3fs\n",
                (double) (end - start) / (double) CLOCKS_PER_SEC);

            LOGI("[RunSFM] Removing %d outliers\n", num_outliers);
        }
                                        ...
                                        bool use_constraints)
{
    ...
    * there is one) */
    ...
    for (int i = 0; i < num_images; i++) {
        if (m_image_data[i].m_camera.m_adjusted) {
            LOGI("[InitializeBundleAdjust] Loading keys for "
                "image %d\n", i);
            m_image_data[i].LoadKeys(false, !m_optimize_for_fisheye);
            m_image_data[i].ReadKeyColors();
            SetTracks(i);
    ...
    for (int i = 0; i < (int) m_point_data.size(); i++) {
        ...
        for (int j = 0; j < num_views; j++) {
            ...
            if (added_order_inv[p.first] == -1) {
                LOGI("Error: added_order_inv[%d] doesn't exist!\n", p.first);
            }

            if (p.first < 0 || p.first >= num_images) {
                LOGI("Error: p.first[%d] out of range\n", p.first);
            }

            if (p.second < 0 || 
                p.second >= (int) m_image_data[p.first].m_keys.size()) {

                    LOGI("Error: p.second[%d,%d] out of range\n", p.second,
    ...
    LOGI("  Avg. proj error [%d projections] = %0.3e\n", n,
        sqrt(error / n));
}
                                               ...
                                               int &max_matches)
{
    ...
    if (parent_idx == -1) {
        LOGI("Error: parent not found a\n");
    }

    LOGI("  accepting image %d (%d / %d)\n", i_best,
                                       ...
                                       bool use_init_focal_only)
{
    ...
    if (m_initial_pair[0] != -1 && m_initial_pair[1] != -1) {
        ...
        LOGI("[BundleAdjust] Setting initial pair to "
            "%d and %d\n", i_best, j_best);
...
#define MIN_MATCHES 80
    ...
    if (i_best == -1 && j_best == -1) {
        if (i_best_2 == -1 && j_best_2 == -1) {
            LOGI("[BundleAdjust] Error: no good camera pairs found!\n");

            if (use_init_focal_only) {
                LOGI("[BundleAdjust] Trying a backup approach...\n");
                BundlePickInitialPair(i_best, j_best, false);
            } else {
                LOGI("[BundleAdjust] Picking first two cameras...\n");
...
int BundlerApp::SetupInitialCameraPair(int i_best, int j_best,
                                       ...
                                       std::vector<ImageKeyVector> &pt_views)
{
	LOGI("BundlerApp::SetupInitialCameraPair start");
	...
	LOGI("BundlerApp::SetupInitialCameraPair xx1");
	...
	LOGI("BundlerApp::SetupInitialCameraPair xx2");
...
#define INITIAL_DEPTH 3.0 // 1000.0 // 3.0 /* was 3.0, fixme! */
    ...
    LOGI("[BundleAdjust] Adding initial matches...\n");
                                        ...
                                        std::vector<ImageKeyVector> &pt_views)
{
    ...
    for (int i = 0; i < num_images; i++) {
        if (m_image_data[i].m_ignore_in_bundle) {
            LOGI("[BundleAdjust] Adding camera %d\n", i);
    ...
    for (int i = 0; i < curr_num_cameras; i++) {
        ...
        LOGI("[EstimateIgnoredCameras] Polishing image %d (%d)\n", image, i);
        ...
        LOGI("[BundleAdjust] Polishing took %0.3fs\n",
...
{
	LOGI("BundlerApp::BundleAdjust start");
    clock_t start = clock();
    ...
    if (num_init_cams == 0) {
        ...
        LOGI("[BundleAdjust] Adjusting cameras "
            "%d and %d (score = %0.3f)\n", 
        ...
        if (m_fix_necker) {
            ...
            * reversal */
            ...
            if (error0 < error1) {
                /* Swap back */
                LOGI("Restoring pre-Necker configuration\n");
...
#if 0
        if (m_initial_pair[0] == -1 || m_initial_pair[1] == -1) {
            LOGI("[BundleAdjust] Error: initial good pair "
                "not provided!\n");
            LOGI("[BundleAdjust] Please specify a pair of "
        ...
        if (good_pair_1 == -1 || good_pair_2 == -1) {
            LOGI("[BundleAdjust] Error: initial pair haven't "
                "been adjusted!\n");
            LOGI("[BundleAdjust] Please specify another pair!\n");
            exit(0);
        }
#endif
    ...
    for (int round = curr_num_cameras; 
        ...
        round++, curr_num_cameras++) {
            ...
            LOGI("[BundleAdjust] max_matches = %d\n", max_matches);
            ...
            * adjustment */
            ...
            LOGI("[BundleAdjust[%d]] Adjusting camera %d "
                "(parent = %d, matches = %d)\n", 
                round, next_idx, 
            ...
            LOGI("[BundleAdjust] Adding new matches\n");
            ...
            curr_num_pts = pt_count;
            LOGI("[BundleAdjust] Number of points = %d\n", pt_count);
            fflush(stdout);
    ...
    LOGI("[BundleAdjust] Bundle adjustment took %0.3fs\n",
                                        ...
                                        double max_proj_error_threshold)
{
    ...
    while (1) {
        LOGI("[RefineCameraParameters] Calling with %d points\n", 
            num_points_curr);
        ...
        LOGI("[RefineCameraParameters] Error: %0.3f\n", 
        ...
        LOGI("[RefineCameraParameters] Threshold = %0.3f\n", threshold);
        for (int i = 0; i < num_points_curr; i++) {
            if (errors[i] < threshold) {
                ...
                if (pt_idxs != NULL) {
                    LOGI("[RefineCameraParameters] Removing point [%d] with "
                        "reprojection error %0.3f\n", pt_idxs[i], 
                        errors[i]);
                } else {
                    LOGI("[RefineCameraParameters] Removing point with "
                        "reprojection error %0.3f\n", errors[i]);
                }
            }
        }
    ...
    LOGI("[RefineCameraParameters] Exiting after %d rounds "
                         ...
                         std::vector<int> &outliers)
{
    ...
    if (r == -1) {
        LOGI("[FindAndVerifyCamera] Couldn't find projection matrix\n");
        return false;
    }

    /* If number of inliers is too low, fail */
    if (r <= MIN_INLIERS_EST_PROJECTION) {
        LOGI("[FindAndVerifyCamera] Too few inliers to use "
            "projection matrix\n");
        return false;
    }
    ...
    LOGI("[FindAndVerifyCamera] Estimated intrinsics:\n");
    matrix_print(3, 3, Kinit);
    LOGI("[FindAndVerifyCamera] Estimated extrinsics:\n");
    matrix_print(3, 3, Rinit);
    matrix_print(1, 3, tinit);
    fflush(stdout);

    /* Check cheirality constraint */
    LOGI("[FindAndVerifyCamera] Checking consistency...\n");
    ...
    for (int j = 0; j < num_points; j++) {
        ...
        if (diff < proj_estimation_threshold_weak) {
            ...
            LOGI("[FindAndVerifyCamera] Removing point [%d] "
                "(reproj. error = %0.3f)\n", idxs_solve[j], diff);
    ...
    if (num_behind >= 0.9 * num_points) {
        LOGI("[FindAndVerifyCamera] Error: camera is pointing "
            "away from scene\n");
        return false;
    }
                                  ...
                                  bool refine_cameras_and_points)
{
    ...
    for (int i = 0; i < num_tracks; i++) {
        ...
        // LOGI("  Connecting existing point [%d] (cam: %d)\n",
        //        pt, image_idx);
    ...
    if (num_pts_solve < m_min_max_matches) {
        LOGI("[BundleInitializeImage] Couldn't initialize\n");
    ...
    LOGI("[BundleInitializeImage] Initializing camera...\n");
    fflush(stdout);
    ...
    if (success) {
        ...
        LOGI("[BundleInitializeImage] Error!  "
            "Pose estimation failed!\n");
    }

    /* **** Finally, start the bundle adjustment **** */
    LOGI("[BundleInitializeImage] Adjusting...\n");
    fflush(stdout);
    ...
    if ((int) inliers.size() < 8 || camera_new.f < 0.1 * data.GetWidth()) {
        LOGI("[BundleInitializeImage] Bad camera\n");
        if (success_out)
            *success_out = false;
    ...
    for (int i = 0; i < num_inliers; i++) {
        ...
        // LOGI("[BundleInitializeImage] Connecting point [%d]\n",
    ...
    LOGI("[BundleInitializeImage] Initializing took %0.3fs\n",
        (double) (end - start) / CLOCKS_PER_SEC);
                                                 ...
                                                 &pt_views) 
{
    ...
    LOGI("[BundleInitializeImageFullBundle] "
        "Connecting existing matches...\n");
        ...
        for (unsigned int j = 0; j < list.size(); j++) {
            ...
            if (GetKey(other,other_idx).m_extra < 0) {
                if (GetKey(other,other_idx).m_extra == -2) {
                    // LOGI("Removed point as outlier\n");
                } else if (GetKey(other,other_idx).m_extra == -1) {
                    // LOGI("Haven't added point yet\n");
                } else {
                    LOGI("Error!  Index = %d\n",
                ...
                if (keys_seen[this_idx] != -1) {
                    LOGI("Error!  Already saw key %d (pt: %d)!\n",
                        this_idx, pt_idx);
                }

                LOGI("  Connecting existing point "
                    "%d => %d [%d] (cam: %d)\n", 
                ...
                * the camera position */
                ...
                if (num_pts_solve > num_keys) {
                    LOGI("Error! Number of points exceeds "
...
{
    LOGI("[BundleRegisterImage] Registering [%dx%d] image\n",
...
#define NUM_NNS 20
        ...
        LOGI("[BundleRegisterImage] Initializing indices...\n");
        ...
        LOGI("[BundleRegisterImage] Set point indices...\n");
        ...
        LOGI("[BundleRegisterImage] Matching images...\n");
        ...
        for (int i = 0; i < NUM_NNS; i++) {
            ...
            /* Remove duplicates */
            LOGI("Found %d matches [before pruning]\n",
                (int) matches.size());

            matches = RemoveDuplicateMatches(matches);
            LOGI("Found %d matches [after pruning]\n",
                (int) matches.size());
        ...
        LOGI("[BundleRegisterImage] Found %d matches\n",
            num_pts_solve);
    ...
    /* **** Solve for the camera position **** */
    LOGI("[BundleRegisterImage] Initializing camera...\n");
    ...
    /* **** Finally, start the bundle adjustment **** */
    LOGI("[BundleRegisterImage] Adjusting [%d,%d]...\n",
...
#define MIN_INLIERS_ADD_IMAGE 16
    if (num_inliers < 0.5 * num_points_final) {
        ...
        LOGI("[BundleRegisterImage] Final camera parameters:\n");
        LOGI("f: %0.3f\n", camera_new->f);
        LOGI("R:\n");
    ...
    LOGI("[BundleRegisterImage] Registration took %0.3fs\n",
        (double) (end - start) / (double) CLOCKS_PER_SEC);
                                          ...
                                          std::vector<ImageKeyVector> &pt_views)
{
    ...
    LOGI("[RemoveBadPointsAndCameras] Pruned %d points\n", num_pruned);
