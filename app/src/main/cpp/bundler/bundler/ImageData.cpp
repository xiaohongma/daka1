...
{
    if (!m_image_loaded) {
        LOGI("[ImageData::UnloadImage] Image hasn't been loaded!\n");
        return;
    }
...
void ImageData::LoadKeys(bool descriptor, bool undistort) {
	...
	LOGI("ImageData::LoadKeys xx1");
    /* Try to find a keypoint file */
    if (!descriptor) {
        std::vector<Keypoint> kps = ReadKeyFile(m_key_name);
		LOGI("ImageData::LoadKeys xx2");
        /* Flip y-axis to make things easier */
        for (int k = 0; k < (int) kps.size(); k++) {
            kps[k].m_y = GetHeight() - kps[k].m_y - 1.0;
        }
    ...
    if (undistort)
        UndistortKeys();
#else
    LOGI("Cannot load keys in demo version\n");
#endif
}
	...
	if (FileExists(bmp_file)) {
	    ...
	    jpeg_file[strlen(m_name) - 1] = 'g';
            ...
            if (FileExists(jpeg_file)) {
                ...
                LOGI("[ImageData::CacheDimensions] Fatal error: "
...
img_t *ImageData::UndistortImage(double theta, double phi, int num_rot) {
    ...
    if (!m_image_loaded) {
	LOGI("[ImageData::UndistortImage] Loading image...\n");
	unload = true;
	LoadImage();
    }
    ...
    if (unload) {
	LOGI("[ImageData::UndistortImage] Unloading image\n");
	UnloadImage();
    }
				   ...
				   double min_line_segment_size) 
{
    ...
    if (!m_image_loaded) {
        LOGI("[ImageData::DetectLineSegments] Loading image...\n");
        LoadImage();
        unload = true;
    }
    ...
    for (int angle_i = 0; angle_i < num_angles; angle_i++) {
        ...
        /* Cleanup */
        LOGI("[ImageData::DetectLineSegments] Detected %d lines\n", 
            num_lines_long);
        LOGI("[ImageData::DetectLineSegments] Freeing %d lines\n", 
            num_lines);
    ...
    if (unload) {
        LOGI("[ImageData::DetectLineSegments] Unloading image\n");
        UnloadImage();
    }
}
				    ...
				    ImageBoundary &b1, ImageBoundary &b2)
{
	...
	if (num_isect == 0) {
	    ...
	    LOGI("[ImageData::LineIntersectsImage] Strange number of "
		   "intersections\n");
	}
	...
	LOGI("[ImageData::LineIntersectsImage] num_isect = %d "
	       "doesn't make sense\n", num_isect);
	return false;
    }
}
...
{
	LOGI("ImageData::ReadKeyColors start");
    double ident[9];
    GetRotationFromSpherical(-0.5 * M_PI, 0.5 * M_PI, ident);
	LOGI("ImageData::ReadKeyColors xx1");
    ...
    LOGI("ImageData::ReadKeyColors xx2");
    int w = GetWidth();
    int h = GetHeight();
    ...
    LOGI("ImageData::ReadKeyColors xx3");
    if (unload)
		UnloadImage();
	LOGI("ImageData::ReadKeyColors end");
}
...
bool ImageData::ReadTracks(int img_idx, std::vector<PointData> &pt_data)
{
    ...
    if (f == NULL) {
	LOGI("[ImageData::ReadTracks] Error opening file %s for reading\n",
	       cam_buf);
	return false;
    }
...
void ImageData::WriteCamera()
{
    ...
    if (f == NULL) {
	LOGI("[ImageData::WriteCamera] Error opening file %s for writing\n",
	       cam_buf);
	return;
    }
...
void ImageData::WriteTracks()
{
    ...
    if (f == NULL) {
	LOGI("[ImageData::WriteTracks] Error opening file %s for writing\n",
	       cam_buf);
	return;
    }
...
void ImageData::ComputeTPSBasis(const std::vector<PointData> &pt_data)
{
    ...
    LOGI("[ImageData::ComputeTPSBasis] Computed basis in %0.3fs\n",
                                 ...
                                 double *g_hist, double *b_hist)
{
    ...
    if (!LoadFeatureWeightMap()) {
        LOGI("[ImageData::ComputeHistogram] Error: weight map for image %s "
               "not found!\n", m_name);
