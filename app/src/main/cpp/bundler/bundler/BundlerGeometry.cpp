                                             ...
                                             int new_image_start) 
{
	...
	LOGI("BundlerApp::ComputeGeometricConstraints start");
	
    /* Read information from files if they exist */
    char *filename = "/storage/sdcard/matcher/constraints.txt";
	if (!overwrite && FileExists(filename)) {
        ...
        if (m_keypoint_border_width > 0) {
            for (int i = 0; i < num_images; i++) {
                for (int j = i+1; j < num_images; j++) {
                    if (!ImagesMatch(i, j))
                        continue;
					LOGI("ImagesMatch, %d, %d", i, j);
                    RemoveMatchesNearBorder(i, j, m_keypoint_border_width);
                }
            }
        }
		LOGI("BundlerApp::ComputeGeometricConstraints xx1");
		...
		LOGI("BundlerApp::ComputeGeometricConstraints xx2");
		...
		LOGI("BundlerApp::ComputeGeometricConstraints xx3");
		...
		LOGI("BundlerApp::ComputeGeometricConstraints xx4");
	    MakeMatchListsSymmetric();
		LOGI("BundlerApp::ComputeGeometricConstraints xx5");
        if (num_images < 40000)
            WriteMatchTableDrew(".ransac");
		LOGI("BundlerApp::ComputeGeometricConstraints xx6");
        // RemoveAllMatches();
	    ComputeTracks(new_image_start);
		LOGI("BundlerApp::ComputeGeometricConstraints xx7");
        // ClearMatches();
        RemoveAllMatches();
        // SetMatchesFromTracks();
		LOGI("BundlerApp::ComputeGeometricConstraints xx8");
#if 1
        /* Set match flags */
...
#endif
		LOGI("BundlerApp::ComputeGeometricConstraints xx9");
        WriteGeometricConstraints(filename);
		LOGI("BundlerApp::ComputeGeometricConstraints xx10");
        if (num_images < 40000)
            WriteMatchTableDrew(".corresp");
		LOGI("BundlerApp::ComputeGeometricConstraints xx11");
		// ComputeMatchPoints(new_image_start);

        // FilterTracks();
    }
	LOGI("BundlerApp::ComputeGeometricConstraints end");
}
...
bool BundlerApp::ComputeTransform(int idx1, int idx2, bool removeBadMatches)
{
    ...
    if (idx1 == idx2) {
	LOGI("[SifterApp::ComputeTransform] Error: computing tranform "
...
void BundlerApp::ComputeTransforms(bool removeBadMatches, int new_image_start) 
{
    ...
    for (int i = 0; i < num_images - 1; i++) {
		...
		LOGI("%% Homography mapping image %d to %d\n", next, i);
		LOGI("%0.6f %0.6f %0.6f\n", H[0], H[1], H[2]);
		LOGI("%0.6f %0.6f %0.6f\n", H[3], H[4], H[5]);
		LOGI("%0.6f %0.6f %0.6f\n", H[6], H[7], H[8]);
    }
    ...
    char buf[256];
    sprintf(buf, "%s/pairwise_scores.txt", m_match_directory);
    FILE *f = fopen(buf, "w");
...
void BundlerApp::CoalesceFeatureDescriptorsMedian()
{
    ...
    LOGI("[SifterApp::CoalesceFeatureDescriptorsMedian] Loading keys\n");
    ...
    LOGI("[SifterApp::CoalesceFeatureDescriptors] Coalescing features\n");
...
void BundlerApp::CoalesceFeatureDescriptors()
{
	...
	LOGI("[SifterApp::CoalesceFeatureDescriptors] Adding features from "
	       "image %d\n", i);
        fflush(stdout);
	...
	if (num_observations[i] != (int) m_point_data[i].m_views.size())
	    LOGI("[SifterApp::CoalesceFeatureDescriptors] "
    ...
    LOGI("[SifterApp::CoalesceFeatureDescriptors] Median squared variance: "
           "%0.3f\n", median);
    fflush(stdout);
                                 ...
                                 double ratio) 
{
    ...
    LOGI("[SifterApp::MatchKeysToPoints] Building tree took %0.3fs\n", 
    ...
    LOGI("[SifterApp::MatchKeysToPoints] Searching tree took %0.3fs\n",
	   (end - start) / ((double) CLOCKS_PER_SEC));

    int num_matches = (int) matches.size();

    LOGI("[MatchKeys] Found %d matches\n", num_matches);
                                 ...
                                 double ratio) 
{
    ...
    LOGI("[SifterApp::MatchPointsToKeys] Building tree took %0.3fs\n", 
	   (end - start) / ((double) CLOCKS_PER_SEC));
    ...
    LOGI("[SifterApp::MatchPointsToKeys] Searching tree took %0.3fs\n",
	   (end - start) / ((double) CLOCKS_PER_SEC));

    int num_matches = (int) matches.size();

    LOGI("[SifterApp::MatchPointsToKeys] Found %d matches\n", num_matches);
    fflush(stdout);
...
void BundlerApp::RemoveMatchesNearBorder(int i1, int i2, int border_width)
{
    ...
    LOGI("[RemoveMatchesNearBorder] Removed %d matches from pair (%d,%d)\n",
           num_removed, i1, i2);
}
...
void BundlerApp::RemoveMatchesNearBottom(int i1, int i2, int border_width)
{
    ...
    LOGI("[RemoveMatchesNearBottom] Removed %d matches from pair (%d,%d)\n",
           num_removed, i1, i2);
}
