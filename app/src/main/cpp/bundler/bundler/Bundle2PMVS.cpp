               ...
               std::vector<camera_params_t> &cameras)
{
    ...
    char buf[2048];
    sprintf(buf, "%s/pre_pmvs.list", output_path);
  ...
  //  fprintf(f_scr, "mkdir -p %s/models/\n", output_path);
    ...
    for (int i = 0; i < num_cameras; i++) {
        ...
        fclose(f);
       // std::string name = images[i].substr(0, images[i].length()-1);

        //fprintf(f_scr,"%s\n",images[i].c_str());
       /* int last_dot = images[i].rfind('/', images[i].length()-1);
        std::string basename = images[i].substr(0, last_dot);
        fprintf(f_scr,"%s/%s/%08d.jpg\n",basename,output_path,count);*/

        int last_slash = images[i].rfind('/');
        int last_dot = images[i].rfind('.');

        assert(last_slash < last_dot);
        std::string im_basename =
                images[i].substr(last_slash+1, last_dot - last_slash - 1);
        //s//td::string pre_path =  images[i].substr(0, last_slash);

        //std::string out =  output_path+"/" + im_basename + ".rd.jpg";
        fprintf(f_scr,"%s/%s.rd.jpg\n",output_path,im_basename.c_str());
        // fprintf(f_scr, "pmvs%s.rd.jpg\n", basename.c_str());
       // fprintf(f_scr, " %s\n", buf);

       // fprintf(f_scr, "mv %s.rd.jpg %s/visualize/%08d.jpg\n",
