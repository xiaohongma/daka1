...
void BundlerApp::BundleAdjustFast() 
{
...
#if 0
    /* Read keypoints */
    LOGI("[SifterApp::BundleAdjust] Reading key colors...\n");
    ReadKeyColors();
#endif
	...
	LOGI("[SifterApp::BundleAdjust] Adjusting cameras "
	       "%d and %d (score = %0.3f)\n", 
	...
	if (good_pair_1 == -1 || good_pair_2 == -1) {
	    LOGI("[SifterApp::BundleAdjust] Error: initial pair haven't "
		   "been adjusted!\n");
	    LOGI("[SifterApp::BundleAdjust] Please specify another pair!\n");
	    exit(0);
	}
#endif
    ...
    while (curr_num_cameras < num_images) {
		...
		LOGI("[SifterApp::BundleAdjust] max_matches = %d\n", max_matches);
		...
		LOGI("[SifterApp::BundleAdjustFast] Registering %d images\n",
			   num_added_images);

		for (int i = 0; i < num_added_images; i++)
			LOGI("[SifterApp::BundleAdjustFast] Adjusting camera %d\n",
			   image_set[i].first);
		...
		for (int i = 0; i < num_added_images; i++) {
			...
			LOGI("[SifterApp::BundleAdjust[%d]] Adjusting camera %d "
			   "(parent = %d)\n", 
			   round, next_idx, 
		...
		LOGI("[SifterApp::BundleAdjust] Adding new matches\n");
		...
		LOGI("[SifterApp::BundleAdjust] Number of points = %d\n", pt_count);
		fflush(stdout);
