...
void BundlerApp::ProcessOptions(int argc, char **argv) 
{
	...
	LOGI("ProcessOptions: %d c=%c optarg=%s\n", optind, c, optarg);
	...
	switch (c) {
	    ...
	    case 'f':
		m_fisheye = true;
		LOGI("Using fisheye lens, param file: %s\n", optarg);
		...
		break;
            case 366:
                ...
                break;
            case 364:
				LOGI("m_match_table, param file: %s\n", optarg);
...
bool BundlerApp::OnInit()
{
    ...
    if (argc >= 2) {
		LOGI("Loading images from file '%s'\n", argv[1]);
		imageList = argv[1];
		load_file = true;
    } else {
		PrintUsage();
    ...
    LOGI("[BundlerApp::OnInit] Processing options...\n");
    ProcessOptions(argc - 1, argv + 1);

    if (m_use_intrinsics && m_estimate_distortion) {
        LOGI("Error: --intrinsics and --estimate_distortion "
               "are incompatible\n");
        exit(1);
    }

    if (m_fixed_focal_length && m_estimate_distortion) {
        LOGI("Error: --fixed_focal_length and --estimate_distortion "
               "are currently incompatible\n");
        exit(1);
    }

    LOGI("[BundlerApp::OnInit] Loading frame...\n");
    ...
    LOGI("[BundlerApp::OnInit] Loading images...\n");
    fflush(stdout);
	LOGI("[BundlerApp::OnInit] xx11\n");
    if (load_file) {
		FILE *f = fopen(imageList, "r");

		if (f == NULL) {
			LOGI("[BundlerApp::OnInit] Error opening file %s for reading\n",
			   imageList);
			exit(1);
		}
		LOGI("[BundlerApp::OnInit] xx12\n");
    ...
    if (m_rerun_bundle) {
        ...
        LOGI("[BundlerApp::OnInit] Reading bundle file...\n");
        ReadBundleFile(m_bundle_file);

        if (m_bundle_version < 0.3) {
            LOGI("[BundlerApp::OnInit] Reflecting scene...\n");
            FixReflectionBug();
        }
	...
	LOGI("[BundlerApp::OnInit] xx14\n");
    if (m_use_constraints) {
        ReadCameraConstraints();
    }

    if (m_ignore_file != NULL) {
        LOGI("[BundlerApp::OnInit] Reading ignore file...\n");
        ReadIgnoreFile();
    }
#endif
    if (m_ignore_file != NULL) {
...
#define MIN_POINT_VIEWS 3 // 0 // 2
        if (!m_run_bundle) {
            ...
            LOGI("[BundlerApp::OnInit] "
                   "Setting up image points and lines...\n");
            SetupImagePoints(/*2*/ MIN_POINT_VIEWS);
            RemoveBadImages(6);

            if (m_point_constraint_file != NULL) {
                LOGI("[BundlerApp::OnInit] Reading point constraints...\n");
                m_use_point_constraints = true;
                ReadPointConstraints();
            }

            LOGI("[BundlerApp::OnInit] Scaling world...\n");

            LOGI("[BundlerApp::OnInit] Computing camera orientations...\n");
        ...
        if (m_add_image_file != NULL) {
            LOGI("[BundlerApp::OnInit] Adding additional images...\n");
            FILE *f = fopen(m_add_image_file, "r");

            if (f == NULL) {
                LOGI("[BundlerApp::OnInit] Error opening file %s for "
                       "reading\n",
                       m_add_image_file);
            } else {
#ifndef __DEMO__
	...
	LOGI("[BundlerApp::OnInit] xx15\n");
    if (m_run_bundle) {
#ifndef __DEMO__
        if (!m_fast_bundle) {
			LOGI("[BundlerApp::OnInit] xx16\n");
            BundleAdjust();
        } else {
			LOGI("[BundlerApp::OnInit] xx17\n");
            BundleAdjustFast();
        }
        if (m_bundle_version < 0.3)
            FixReflectionBug();
...
int bundler(int argc, char **argv)
//int test()
{
    // mtrace();
	/*int argc = 4;
	...
	argv[0] = "Bundler";
	argv[1] = "/storage/sdcard/matcher/list.txt";
	argv[2] = "--options_file";
	argv[3] = "/storage/sdcard/matcher/options.txt";*/
