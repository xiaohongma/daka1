/*	Copyright (c) 2013, Robert Wang, email: robertwgh (at) gmail.com
	All rights reserved. https://sourceforge.net/descriptors/ezsift
...
std::vector< std::vector<float> > compute_gaussian_coefs(
	int nOctaves, 
	int nGpyrLayers)
{
	...
	float * & hist)
{
	...
	for (int i = -win_radius; i <= win_radius; i ++) // rows
	{
		...
		for (int j = -win_radius; j <= win_radius; j ++) // columns
		{
			...
			weight = expf( -1.0f * ((i-d_kptr) * (i-d_kptr) + (j-d_kptc) * (j-d_kptc))
				/ (2.0f * sigma * sigma));
	...
	std::list<MatchPair> & match_list)
{
	...
	list<MatchPair>::iterator descriptors;
	int match_idx = 0;
	for (descriptors = match_list.begin(); descriptors != match_list.end(); descriptors ++)
	{
		printf("\tMatch %3d: (%4d, %4d) -> (%4d, %4d)\n", match_idx, descriptors->r1, descriptors->c1, descriptors->r2, descriptors->c2);
		match_idx ++;
	}
#endif
          ...
          // fgets(buf, 1024, fp);
           ...
           if (line < 6) {
               fprintf(fp, 
                   "%d %d %d %d %d %d %d %d %d %d "
                   "%d %d %d %d %d %d %d %d %d %d", 
                   (int)(it->descriptors[line*20+0]), (int)(it->descriptors[line*20+1]), (int)(it->descriptors[line*20+2]), (int)(it->descriptors[line*20+3]), (int)(it->descriptors[line*20+4]), (int)(it->descriptors[line*20+5]),
					   (int)(it->descriptors[line*20+6]), (int)(it->descriptors[line*20+7]), (int)(it->descriptors[line*20+8]), (int)(it->descriptors[line*20+9]),
                   (int)(it->descriptors[line*20+10]), (int)(it->descriptors[line*20+11]), (int)(it->descriptors[line*20+12]), (int)(it->descriptors[line*20+13]), (int)(it->descriptors[line*20+14]),
                   (int)(it->descriptors[line*20+15]), (int)(it->descriptors[line*20+16]), (int)(it->descriptors[line*20+17]), (int)(it->descriptors[line*20+18]), (int)(it->descriptors[line*20+19]));
			   fprintf(fp,"\n");
               //descriptors += 20;
           } else {
			   fprintf(fp, "%d %d %d %d %d %d %d %d",
                   (int)(it->descriptors[line*20+0]), (int)(it->descriptors[line*20+1]), (int)(it->descriptors[line*20+2]), (int)(it->descriptors[line*20+3]),
						   (int)(it->descriptors[line*20+4]), (int)(it->descriptors[line*20+5]), (int)(it->descriptors[line*20+6]), (int)(it->descriptors[line*20+7]));
               //descriptors += 8;
           }
       }
	...
	std::list<MatchPair> & match_list)
{
	...
	std::list<MatchPair>::iterator descriptors;
	for (descriptors = match_list.begin(); descriptors != match_list.end(); descriptors ++)
	{
		MatchPair mp;
		mp.r1 = descriptors->r1, mp.c1 = descriptors->c1;
		mp.r2 = descriptors->r2, mp.c2 = descriptors->c2 + image1.w;
